...
const LongExploreMovieCard = ({
  ...
  return (
      ...
      <div
        ...
        className="item aspect-9/16 h-full rounded-xl relative"
      >
        <div className="h-full rounded-xl -z-1 aspect-[9/16]">
         <Link to={`/watch?v=${data.movieId}`}>
             <img
            className="rounded-xl h-full w-full -z-1 object-cover"
        ...
        <div
          className={`h-[0%] w-[0%] flex flex-col rounded-xl overflow-hidden aspect-[9/16]  ease duration-250 ${isHovered && "h-[125%] w-[120%] opacity-100 z-1"} absolute opacity-0 left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 bg-[#171717]`}
        >
         <Link className="h-[80%] w-full" to={`/watch?v=${data.movieId}`}>
              <img
                src={`https://image.tmdb.org/t/p/w500${data.posterImage}`}
                alt=""
                loading="eagar"
          ...
          <div className="py-3 xl:py-3 px-5 flex-1 min-h-0">
            <div className="h-full w-full flex flex-col justify-between">
              <div className="play-like items-center h-[35%] flex gap-3">
                <Link to={`/watch?v=${data.movieId}`}>
                    <div className="rounded-full bg-white h-full  p-[5px] flex justify-center items-center cursor-pointer">
                              <Play fill="black" className="w-full h-full" />
                      </div>
                </Link>  
                <div onClick={()=>handleWhislist(data.movieId,data.poster,data.title,data.year,data.genres,data.popularity)} className="rounded-full  h-[90%] aspect-square  cursor-pointer group">
                  <Heart className={`w-full h-full ${wishlist?.includes(String(data.movieId)) && 'fill-[#D9232E] text-[#D9232E]'}  ease-in duration-75`} />
                </div>
              </div>
              <div className="h-[65%] flex w-full  font-[my-font-Md] flex-col gap-1 items-start justify-end">
